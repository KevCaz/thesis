...
L'analyse des distributions d'espèce est riche quand ol'anaylse a de multiple approche. Bien entendu dans un premier temps, l'examen sur de multiple aure de répartition est un premier pas pour y déceler des phénomènes répétés nécessaire à l'élaboration des th.éories [@macarthur1972geographical]. Mais c'est bien dans l'examen des distribution d'échelle à différentséchelle de temos et d'espace, dans les variations que les infornations abondent car si ces variations sont par exemple reflétées dans els changelmnts climatiques, cela justifie l'espoir de comprendre  l'écvolution des distributin d'espèces dans un contextde chanmgemnt climatique. De plus l'examen d'une distribution est aussi à faire par comprarison avec d'autre espèces niontememnt les espèces en interaction pour voir si la biologie laisse ces emoreinte dans la géométrie de ces aires de répartition. C'est ainsi que dans ma thèse je proporse de regarder l'intersaection des aires d'un ensemble d eproie pour conprendre ce que la co-occurrrence peut nous apprendre. Une connaissance approfondie de la dynamique des distributions d'espèce est aussi un indice de norte qui a des enjeux appliqués divers.
...
Cette approche s'apparentent à la recherche de facteurs limitants qui sont le reflets de contrinate physiologiques. On peut encore cité l'exemple fourni par @Engelbrecht2007 qui ont montrés qu'au niveau du Panama la distribution local et régionale de 48 espèces d'arbres étaint bien expliqué par la sensibilité à la sécheresse, donc à une variation dans la disponibilité d'une ressource. Ces corrélations convaincantes fondent les modèles de distributions d'espéces (SDM enréférence au terme anglais utilisé souvent dans le reste de la thèse) qui cherche à faire correspondre les variables climatiques aux données de co-occurrence [@Elith2006, @Elith2009a]. Le succès récents de ces apporches a l'abondance des données climatiques comme cellles porposées librement par WorldClim (données disponible en ligne [http://worldclim.org](http://worldclim.org), @Hijmans2005) et la relative faciliter d'abtonr des donn.es de co-occurrence qui tentent à être égalment disponble en ligne comme le portail de données sur la biodiversité à l'échelle mondiale GBIF (Global Biodiversity Information Facility, [http://www.gbif.org](http://www.gbif.org)) malgré des biais lié à des efforts différents dans les différentes régions du globe [@Beck2014a].  Le succès repose sur les besooin prédictif dans un contexte de changement climatique qui conduisent à un effort de recherche important dans le domaine \cite{Thomas2004,Thuiller2011}.
