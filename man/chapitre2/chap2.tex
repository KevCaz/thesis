...
Au chapitre précédent, j'ai mis en évidence l'impact potentielle des interactions sur la présence locale des espèces dans le cadre de la théorie de la biogéographie des îles. Afin de faire un pas en avant faire le traitement de données typique en biogégraphie~: les données de présence et d'absence à large échelles des espèces, je me suis confronté auz porblèmes de co-occurrence des espèces en intéractions.

La co-occurence observée dans les données est simplement le nombre total de sites où les espèces sont présentes ensemble rapporté au nombre totale de sites étudiés. Pour pouvoir allez plus loin, nous avons défini une mesure de co-occurence sous l'hypothèse d'indépendance de deux espèces. Cela signigfie que nous prenons l'occurrence respective des deux espèces et que nous les mulitplions pour obtenir notre seconde mesure de co-occurrence sous cette hypothèse. Grâce à la comparaison entre ces deux valeurs nous avons pu illustrer, dans l'article présenté ci-dessous, cinq grands principes relatifs à la co-occurence des espèces en interaction :

1. \textbf{Les interactions directes entre deux espèces affectent la co-occurence}. C'est une transposition directe du chapitre précédent sur la mesure de co-occurrence~: s'il existe un lien entre deux espèces, leur probabilité d'être présentes simultannément dans une localité différe de la probabilité attendue si elle se recontraient aléatoirement.

2. \textbf{Les interactions indirectes modifient la probabilité de co-occurence}. Malgrè l'abscence d'interaction directe entre deux espèces, ces dernières peuvent néanmoins être liées par une ou plusieurs autres espèces, l'interaction est alors qualifiée d'indirecte. Si les conséquences des interactions directes se propagent à travers le réseau via ces relations indirectes, il est alors possible que répartition d'une espèce soit affectée par une autre espèce avec laquelle aucune interaction directe n'est constatée.
...
Pour ce second article, le contexte est particulier~: Dominique Gravel a été invité à un numéro spécial de la revue scientifique \textit{Theoritical Ecology} sur les réseaux écologiques. Dominique Gravel m'a alors proposé de travailler sur le prolongement de la reflexion menée au premier chapitre et de l'appliquer sur les données de co-occurrence. J'ai alors conceptualisé un modèle probabiliste pour tenter de comprendre comment les interactions peuvent affecter la co-occurrence. Je me suis occupé de toute la partie modèle et des figures. Dominique Gravel a écrit la majeure partie de l'introduction et de la discussion et est devenu dernière auteur. Ce travail a aussi fortement bénéficié de l'apport de Niclas Mouquet et de  Miguel B. Ara\'ujo qui sont alors devenus co-auteurs.


\subsection{Traduction du résumé de l'article publié}

L'étude de la co-occurrence des espèces a été essentielle en écologie des communautés depuis la fondation de la discipline. Cependant, les données de co-occurende sont une source d'infornation néglidée par les modèles de distribution d'espèces et les biogéographer se demandent encore quels sont les effets des interactions à traver les échelles spatiales. Nous soutenons l'idée qu'une théori de la co-occurrence des espèce au sein d'un réseau écologique est nécesssaire pour mieux interpréter les données de co-occurrence, pour formuler des hypothèses sur ls méchanismes d'assemblage des communautés et pour étendre l'analyse des distributions d'espèces qui se concentrent actuellemnt sur la relation entre les occurrence et les facteurs abiotiques. L'objectif principale de cet article est de fournir les premières briques 
...
\emph{Les sections qui suivent sont celles de l'articles publié.}
