...
Peterson2011, Boulangeat2012}. The question of whether such
...
(\citealt{Wootton1994}). For instance, plant and carnivore abundances are expected to correlate across a productivity gradient
(\citealt{Hairston1960, Oksanen1981}) because of top-down control on the herbivore population. Similarly, the propagation of indirect interactions has been studied in more complex interaction networks (\citealt{Yodzis1988}). Indirect interactions could reverse the net interaction in a surprising way, such that predator-prey abundances could be positively related (\citealt{Montoya2009Press}). Empirical analysis of co-occurrence for several taxa has shown that they are usually asymmetric (Araújo et al. 2011), such that a species distribution tended to be nested within the distribution of other (\textit{e.g.} predator-prey distributions; \citealt{Holt2009, Gravel2011}). In such a case, even if the co-distribution signature is quite understood, available methods will likely fail at using this piece of information to improve forecasts.
...
Stouffer2007, Stouffer2010}). We use the same approach as in \eqref{sp2ind} and get the subsequent equation:
...
\citealt{Araujo2011Using, Boulangeat2012}). The reason is simple, usually the observed
