...
difficile de trouver des signaux de co-occurrence\footenote{Il s'agit de
...
est de savoirt si la co-occurrence des espèces contiennent une trace des
interactions. L'hypothèse conventionnelle afirme que les interactions
négatives et positives devraient respectivement conduire à des répulsions et
des attractions. La rareté des jeux de données combinant des distributions
à larges échelles avec des données d'interactions a longentemps empéché
les biogéographes d"obtenir des idées clair sur ce problème. Pour répondre
à cette question, nous avons utiliser cinq jeux de données sur un large
gradient de conditions climatiques avec les interactions écologiqyes,
représentant un total de 793 espèces et 354015 observations de co-occurremne.
Nous avons comparé la co-occurrence des pairs d'espèces intéragissant avec
celle de paire d'espèces n'interagissant pas en intégrant ou non kes
variables abiotiques. Nous avons trouvé un effet des interaction sur la
distribution des espèce de co-occurrence brute mais pas de preuse d'un tel
effet lorsque les variables abiotiques étaient ntégrées. De plus, nous avons
trouvés que certaines propriétés du réseau d'interactions pouvait caché l'impact
des interactions sur la co-occurrence. Nous avons montré que plus le nombre
d'interaction qu'une espèces entretennaientétait grand, plus notre capacté
à detecter le signal dand les données d'interactions staitqiues étaient faibles.
De plus, nous démontrons clairement que le signal de co-occurrence entre un
prédateur et ses proies disparait quand la propoortions de sites couvert par
couvert par son ensemble de proie croit. Dans un
contexte où les écosystèmes sont fortement perturbés par l'activité humaine, nous
ésultats insistent sur le besoin d'intégrer les processus écologiques dans les
